...
async function runVerification() {
    ...
    console.log('--- SCENARIO 1: Coordinator Registration & Area Assignment ---');
    {
        ...
        (await db.prepare('UPDATE users SET email_verified = 1 WHERE id = ?').run(newCoordId));
        const verifiedUser = (await db.prepare('SELECT * FROM users WHERE id = ?').get(newCoordId));
        ...
        const hrUser = (await db.prepare("SELECT id FROM users WHERE role = 'HR' LIMIT 1").get());
        (await db.prepare(`
            INSERT INTO coordinator_area_assignments (user_id, area_id, assigned_by, assigned_at, is_current, remarks)
            VALUES (?, 1, ?, UTC_TIMESTAMP(), 1, 'Initial assignment on onboarding')
        `).run(newCoordId, hrUser.id));

        (await db.prepare(`
            UPDATE users SET status = 'active', approved_by = ?, approved_at = UTC_TIMESTAMP() WHERE id = ?
        `).run(hrUser.id, newCoordId));

        const activatedUser = (await db.prepare('SELECT * FROM users WHERE id = ?').get(newCoordId));
        assert(activatedUser.status === 'active', 'User status becomes active following HR approval');

        const currentAssignment = (await db.prepare('SELECT * FROM coordinator_area_assignments WHERE user_id = ? AND is_current = 1').get(newCoordId));
    ...
    console.log('\n--- SCENARIO 2: Absence & Back-to-Work Clearance Workflow ---');
    {
        ...
        const emp = (await db.prepare("SELECT * FROM employees WHERE status = 'active' AND area_id = 1 LIMIT 1").get());
        ...
        (await db.prepare(`
            ...
            VALUES (?, ?, '08:00', '17:00', 0, 1, UTC_TIMESTAMP())
        ON DUPLICATE KEY UPDATE shift_start = VALUES(shift_start), shift_end = VALUES(shift_end), is_rest_day = VALUES(is_rest_day)
        `).run(emp.id, testDate));
        const sched = (await db.prepare('SELECT * FROM employee_schedules WHERE employee_id = ? AND work_date = ?').get(emp.id, testDate));
        assert(sched.shift_start === '08:00', 'Shift schedule saved successfully');

        // 2. Mark attendance as absent -> triggers BTW workflow
        const coord = (await db.prepare("SELECT id FROM users WHERE role = 'COORDINATOR' AND status = 'active' LIMIT 1").get());
        (await db.prepare(`
            INSERT INTO employee_attendance (employee_id, schedule_id, work_date, status, remarks, recorded_by, recorded_at)
            VALUES (?, ?, ?, 'absent', 'Unreported illness', ?, UTC_TIMESTAMP())
        ON DUPLICATE KEY UPDATE status = VALUES(status)
        `).run(emp.id, sched.id, testDate, coord.id));

        // Run BTW absence trigger service
        const btwCase = (await handleEmployeeAbsence(emp.id, testDate, coord.id, 'Unreported illness'));
        assert(btwCase != null && btwCase.id > 0, 'Back-to-Work clearance case automatically created');

        const createdCase = (await db.prepare('SELECT * FROM btw_cases WHERE id = ?').get(btwCase.id));
        ...
        (await db.prepare(`
            ...
            VALUES (?, ?, 'COORDINATOR', 'Employee contacted me with medical certificate.', 'comment', UTC_TIMESTAMP())
        `).run(createdCase.id, coord.id));

        // 4. HR reviews and approves with return date
        const hr = (await db.prepare("SELECT id FROM users WHERE role = 'HR' LIMIT 1").get());
        const returnDate = '2026-10-16';
        (await db.prepare(`
            UPDATE btw_cases 
            SET status = 'approved', hr_reviewer_id = ?, hr_decision = 'APPROVED', 
                hr_remarks = 'Medical clearance accepted', decision_at = UTC_TIMESTAMP(), 
                authorized_return_date = ?, updated_at = UTC_TIMESTAMP()
            WHERE id = ?
        ...
        const decidedCase = (await db.prepare('SELECT * FROM btw_cases WHERE id = ?').get(createdCase.id));
        ...
        const notif = (await db.prepare('SELECT * FROM notifications WHERE user_id = ? ORDER BY id DESC LIMIT 1').get(createdCase.coordinator_id));
    ...
    {
        const emp = (await db.prepare("SELECT * FROM employees WHERE status = 'active' LIMIT 1").get());
        const coord = (await db.prepare("SELECT id FROM users WHERE role = 'COORDINATOR' AND status = 'active' LIMIT 1").get());
        const hr = (await db.prepare("SELECT id FROM users WHERE role = 'HR' LIMIT 1").get());

        // 1. Coordinator reports concern
        const concernRes = (await db.prepare(`
            INSERT INTO concern_reports (employee_id, area_id, category, description, reported_by, status, created_at, updated_at)
            VALUES (?, ?, 'Tardiness', 'Employee late 3 days in a row', ?, 'submitted', UTC_TIMESTAMP(), UTC_TIMESTAMP())
        ...
        (await db.prepare(`
            ...
            VALUES (?, ?, 'HR', 'Please conduct a 1-on-1 counseling session and have employee sign counseling log.', 'instruction', UTC_TIMESTAMP())
        `).run(concernId, hr.id));

        (await db.prepare("UPDATE concern_reports SET status = 'for_employee_reporting' WHERE id = ?").run(concernId));

        // 3. Mark resolved
        (await db.prepare(`
            UPDATE concern_reports SET status = 'resolved', updated_at = UTC_TIMESTAMP() WHERE id = ?
        `).run(concernId));

        const resolvedConcern = (await db.prepare('SELECT * FROM concern_reports WHERE id = ?').get(concernId));
    ...
    console.log('\n--- SCENARIO 4: Manpower Request & Deployment Workflow ---');
    {
        const coord = (await db.prepare("SELECT id FROM users WHERE role = 'COORDINATOR' AND status = 'active' LIMIT 1").get());
        const hr = (await db.prepare("SELECT id FROM users WHERE role = 'HR' LIMIT 1").get());
        const pos = (await db.prepare("SELECT id FROM positions LIMIT 1").get());

        // 1. Submit request for 2 workers
        const mpRes = (await db.prepare(`
            INSERT INTO manpower_requests (area_id, requested_by, status, deployment_date, shift_details, reason, created_at, updated_at)
            VALUES (1, ?, 'submitted', '2026-11-01', '08:00 - 17:00', 'Peak volume surge', UTC_TIMESTAMP(), UTC_TIMESTAMP())
        `).run(coord.id));
        ...
        const summary1 = (await getManpowerSummary(reqId));
        ...
        const summary2 = (await getManpowerSummary(reqId));
        ...
        const empInsert = (await db.prepare(`
            ...
            VALUES (?, 'Incoming', 'Worker', ?, 1, ?, ?, '2026-11-01', 'active', UTC_TIMESTAMP(), UTC_TIMESTAMP())
        ...
        (await db.prepare(`
            ...
            SET status = 'confirmed', employee_id = ?, confirmed_by = ?, confirmed_at = UTC_TIMESTAMP(),
                actual_position_id = ?, updated_at = UTC_TIMESTAMP()
            WHERE id = ?
        `).run(newEmpId, coord.id, pos.id, allocId));

        (await syncRequestStatus(reqId));
        const summary3 = (await getManpowerSummary(reqId));
        ...
        const activeWorker = (await db.prepare('SELECT * FROM employees WHERE id = ?').get(newEmpId));
    ...
    console.log('\n--- SCENARIO 5: Coordinator Own Attendance with Server Timestamps ---');
    {
        ...
        (await db.prepare(`
            ...
            VALUES (?, ?, '07:45', ?)
            ON DUPLICATE KEY UPDATE time_in = VALUES(time_in), time_in_server = VALUES(time_in_server)
        ...
        const rec = (await db.prepare('SELECT * FROM coordinator_attendance WHERE user_id = ? AND work_date = ?').get(coord.id, testWorkDate));
        ...
        const hrQuery = (await db.prepare(`
    ...
    console.log('\n--- SCENARIO 6: Admin System-Wide Audit & Activity Review ---');
    {
        ...
        (await logAction(admin.id, 'ADMIN_TEST_VERIFY', 'system', 1, { action: 'test audit' }));
    ...
    {
        const emp = (await db.prepare("SELECT id FROM employees WHERE status = 'active' LIMIT 1").get());
        const testDate = '2026-11-20';

        // 1. Unique constraint on (employee_id, work_date)
        (await db.prepare(`
            INSERT INTO employee_attendance (employee_id, work_date, status, recorded_by)
            VALUES (?, ?, 'present', 1)
        ON DUPLICATE KEY UPDATE status = VALUES(status)
        `).run(emp.id, testDate));
        ...
        const count = (await db.prepare('SELECT COUNT(*) as count FROM employee_attendance WHERE employee_id = ? AND work_date = ?').get(emp.id, testDate)).count;
        assert(count === 1, 'Duplicate attendance prevented by UNIQUE(employee_id, work_date) constraint');

        // 2. Duplicate BTW case prevention
        const btw1 = (await handleEmployeeAbsence(emp.id, testDate, 1, 'First save'));
        const btw2 = (await handleEmployeeAbsence(emp.id, testDate, 1, 'Repeated save / page refresh'));
...
if (require.main === module) {
    (async () => {
        if (!/^taascor_test_/.test(process.env.DB_NAME || '')) throw new Error('Verification requires a disposable DB_NAME beginning with taascor_test_.');
        await initializeDb();
        await require('./db/seed').createInitialAdmin();
        await require('./db/seed').seedSampleData();
        await runVerification();
    })().catch(err => { console.error(err); process.exitCode = 1; }).finally(closeDb);
}
